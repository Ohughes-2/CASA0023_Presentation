...
1. Sentinel-2 has nearly the same resolution as CBERS-4A (10 vs 8 m) and returns more often (5 vs 31 days)<sup>3</sup>
2. We only really need a binary classification, not the object-based classification of urban typology
3. Clouds are common in the rainforest and block optical EO<sup>2</sup>
4. Sentinel-1 carries C-band SAR and returns every 6 days<sup>4</sup>
5. Moya et al. demonstrated that pixel-wise classification by SAR can identify changes quickly in Lima, Peru<sup>5</sup>
...
3. Compare each new analysis to the previous ones and alert local officials to changes
4. Work with officials to determine suitable predicted probability threshold to balance TPR and FPR
5. Experimentally determine suitable opening and closing kernel size and minimum blob size if neccessary
